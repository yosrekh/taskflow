...
1. قم بإنشاء ملف الحزمة الجديدة:
   ...
   ```
2. ارفع الملف المضغوط إلى مجلد النطاق الفرعي عبر File Manager وقم بفك الضغط لاستبدال ملفات الكود (ملف `.env` الموجود بالخارج لن يتأثر إطلاقاً).
3. في حال وجود ترحيلات لقاعدة البيانات جديدة داخل `db/migrations/`:
   - افتح الملفات الجديدة (مثل `003_xxx.sql`).
   - انسخ محتواها ونفّذه عبر تبويب **SQL** في phpMyAdmin.

---

## 11. النسخ الاحتياطي الدوري (Backups)
...
2. **نسخ الملفات**:
   - من cPanel، استخدم **Backup Wizard** أو توجّه إلى **File Manager**، حدد ملفات مجلد النطاق الفرعي بالإضافة لملف `.env`، واضغط **Compress** لحفظها بصيغة ZIP وتحميلها محلياً.
